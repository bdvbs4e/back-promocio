...
const validateCredentials = async (req, res) => {
    ...
    try{
      const login =  await pool.db('Promoción').collection('user').findOne({ email: datos.email, pass: hashedPassword });
      if (login) {
        ...
        await pool.db('Promoción').collection('log_login').insertOne({ email: datos.email, role: login.role, date: currentDateTime });
  ...
  const Signup = async (req, res) => {
    ...
    try {
        const userFind = await pool.db('Promoción').collection('user').findOne({ email: datos.email });
        if (userFind) {
            res.status(409).json({ message: `El usuario con el correo: ${datos.email} ya está creado` });
        } else {
            const newUser = await pool.db('Promoción').collection('user').insertOne({ email: datos.email, pass: hashedPassword, role: datos.role });
            const userId = newUser.insertedId;
            await pool.db('Promoción').collection('user_info').insertOne({ user_id: userId, nombre: datos.name, celular: datos.phone, fecha_nac: datos.birthdate, cedula: datos.idNumber, ciudad: datos.city });
